...
WITH media_divisao AS (
	SELECT
		emp.lotacao,
		emp.lotacao_div,
		ROUND(AVG(v.total_venc - COALESCE(d.total_desc, 0)), 2) AS media
	FROM empregado emp
	INNER JOIN (
		SELECT
			ev.matr,
	...
	LEFT JOIN (
		...
		GROUP BY ed.matr
	) d ON emp.matr = d.matr
	GROUP BY emp.lotacao, emp.lotacao_div
), media_max_departamento AS (
	SELECT
		m.lotacao,
		MAX(m.media) AS media_maxima
	FROM media_divisao m
	GROUP BY m.lotacao
)
SELECT
	...
	m.media AS media
FROM media_divisao m
INNER JOIN media_max_departamento mx ON m.lotacao = mx.lotacao AND m.media = mx.media_maxima
INNER JOIN departamento d ON m.lotacao = d.cod_dep
INNER JOIN divisao dv ON m.lotacao_div = dv.cod_divisao
ORDER BY media DESC;