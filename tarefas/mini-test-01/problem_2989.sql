...
FROM (
	SELECT
		v.lotacao,
		v.lotacao_div,
		ROUND(SUM(v.total_venc) - COALESCE(SUM(de.total_desc), 0), 2) AS salario
	FROM (
		SELECT
			emp.matr,
			emp.lotacao,
			emp.lotacao_div,
			COALESCE(SUM(venc.valor), 0) AS total_venc
		FROM empregado emp
	...
	LEFT JOIN (
		SELECT
			...
			emp.lotacao,
			emp.lotacao_div,
			COALESCE(SUM(des.valor), 0) AS total_desc
		FROM empregado emp
		LEFT JOIN emp_desc ed ON emp.matr = ed.matr
		LEFT JOIN desconto des ON ed.cod_desc = des.cod_desc
		GROUP BY emp.matr, emp.lotacao, emp.lotacao_div
	) AS de ON v.matr = de.matr
	GROUP BY v.matr, v.lotacao, v.lotacao_div
) AS s
INNER JOIN departamento AS dep ON s.lotacao = dep.cod_dep
INNER JOIN divisao AS dv ON s.lotacao_div = dv.cod_divisao
GROUP BY dv.nome, dep.nome
ORDER BY media DESC
